...
publications %>%
  ...
  manual_page_break_after_row("ref2print", "Evidence that inconsistent gene prediction can mislead analysis of") %>%
  knitr::kable()
```
...
cv_entries %>% 
  filter(type == 'talk') %>% 
  select(year_begin, what) %>% 
  manual_page_break_after_row("what", "Annual Meeting of the Phycological Society of America") %>%
  kable()
```
