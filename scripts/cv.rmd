...
publications %>%
  ...
  manual_page_break_after_row("ref2print", "Life on the edge: Hawaiian model for coral evolution") %>%
  manual_page_break_after_row("ref2print", "genomes reveal adaptive evolution of functions related to coral-dinoflagellate symbiosis") %>%
  knitr::kable()
```
...
cv_entries %>% 
  filter(type == 'talk') %>% 
  select(year_begin, what) %>%
  manual_page_break_after_row("what", "Carnegie Department of Plant Biology,") %>%
  kable()
```
...
cv_entries %>% 
  filter(type == 'poster') %>%
  select(year_begin, what) %>%
  manual_page_break_after_row("what", "21-24th August 2023") %>%
  kable()
```
