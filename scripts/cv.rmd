...
publications %>%
  ...
  manual_page_break_after_row("ref2print", " Comparative genomics supports that Brazilian bioethanol") %>%
...
cv_entries %>% 
  ...
  mutate(what = if_else(!is.na(additional_info),
                        ...
                        str_c(what, ", ", where))) %>% 
  select(year_begin, what) %>%
  manual_page_break_after_row("what", "Core Facility Utilization Application 2022") %>%
  kable()
```
...
cv_entries %>% 
  filter(type == 'talk') %>% 
  select(year_begin, what) %>%
  manual_page_break_after_row("what", "13-14 December 2023, University of Queensland, Australia.") %>%
  kable()
```
