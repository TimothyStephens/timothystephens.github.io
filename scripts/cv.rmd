...
publications %>%
  ...
  manual_page_break_after_row("ref2print", "binding protein that functions to relieve Fur-mediated repression of gene expression in") %>%
  manual_page_break_after_row("ref2print", "splicing generates evolutionary novelty in the photosynthetic amoeba") %>%
  manual_page_break_after_row("ref2print", "PhySortR") %>%
  knitr::kable()
```
...
cv_entries %>% 
  ...
  mutate(what = if_else(!is.na(additional_info),
                        ...
                        str_c(what, ", ", where))) %>% 
  select(year_begin, what) %>%
  #manual_page_break_after_row("what", "Funds to continue development of our coral health monitoring toolkit.") %>%
  kable()
```
...
cv_entries %>% 
  ...
  select(year, what) %>% 
  manual_page_break_after_row("what", "Duties included assisting students with the completion of set questions and marking of assignments") %>%
  kable()
```
...
cv_entries %>% 
  filter(type == 'talk') %>% 
  select(year_begin, what) %>%
  manual_page_break_after_row("what", "10th January 2023") %>%
  kable()
```
...
cv_entries %>% 
  filter(type == 'poster') %>%
  select(year_begin, what) %>%
  manual_page_break_after_row("what", "13th Jul 2016") %>%
  kable()
```
