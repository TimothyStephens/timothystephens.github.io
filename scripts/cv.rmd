...
publications %>%
  ...
  manual_page_break_after_row("ref2print", "Multiple waves of viral invasions in") %>%
  manual_page_break_after_row("ref2print", "Commonly misunderstood parameters of NCBI BLAST") %>%
  knitr::kable()
```
...
cv_entries %>% 
  ...
  select(year_begin, what) %>% 
  arrange(desc(year_begin)) %>%
  manual_page_break_after_row("what", "Botany Department, Biosciences Institute") %>%
  kable()
```
