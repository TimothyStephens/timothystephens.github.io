...
publications %>%
  ...
  manual_page_break_after_row("ref2print", "generates evolutionary novelty in the photosynthetic amoeba") %>%
  knitr::kable()
```
...
cv_entries %>% 
  filter(type == 'talk') %>% 
  select(year_begin, what) %>%
  manual_page_break_after_row("what", "4th Institute for Food, Nutrition, and Health") %>%
  kable()
```
